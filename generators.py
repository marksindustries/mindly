...
# def summary_from_context(context: str) -> str:
#     chain = SUMMARY_PROMPT | _chat(0.2) | StrOutputParser()
#     return chain.invoke({"context": context}).strip()

# def interview_qs_from_context(context: str, count: int = 10) -> str:
#     chain = INTERVIEW_QS_PROMPT | _chat(0.2) | StrOutputParser()
#     return chain.invoke({"context": context, "count": count}).strip()

# def quiz_from_context(context: str, count: int = 5):
#     chain = QUIZ_JSON_PROMPT | _chat(0.2) | StrOutputParser()
#     raw = chain.invoke({"context": context, "count": count}).strip()
#     try:
#         data = json.loads(raw)
...
#     except Exception:
#         pass
#     # naive fallback parse
#     qs, blocks = [], [b.strip() for b in raw.split("\n\n") if b.strip()]
#     for b in blocks:
